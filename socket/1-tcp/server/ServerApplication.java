...
class Client extends Thread {
	...
	static int currentTime = 1;	// 서버 연결 시간
	int scode;					// 상태 코드
	...
	public void run() {
		try {
			...
			System.out.println("Server > " + this.socket.toString());
	...
	public void getClientList() {
		...
		if(server.clients.size() != clientList.size()) {
			clientList.clear();
	...
	/* 연결이 종료된 클라이언트의 CID와 IP주소를 리스트에서 삭제하는 메소드 */
	public void removeClient(String removeCid) {
		String clientId = null;
		for(int i = 0; i < server.clients.size(); i++) {
			clientId = server.clients.get(i).cid;
			
			if(clientId.equals(removeCid) || clientId == null) {
				clientList.remove(i);
			}
		}
	}
	...
	public void resMessage(int scode, String value) {
		try {
			dout.writeUTF("Res///" + scode + "///" + value + "///END_MSG");
		} catch (IOException e) {
			System.out.println("입출력 예외 발생...");
		}
	}
