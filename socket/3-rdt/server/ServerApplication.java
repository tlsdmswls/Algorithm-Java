...
class Simulator {
	...
	// Response message
	public void resWriteTest(String msg) throws IOException {
		int if_write = rd.nextInt(10);
		if(if_write < 7) {
			c.dout.writeUTF(msg);
			status = "NoLoss";
		}
		else { c.dout.writeUTF(msg); status = "Loss"; }
	}
	
	/* 난수가 0~6이면 쓰기 동작 수행, 7~9면 무시 */
	// Ack message
	public void ackWrite(int num_req) {
		int if_write = rd.nextInt(10);
		if(if_write < 7) {
			c.ackMessage(num_req);
			status = "NoLoss";
		}
		else { c.ackMessage(num_req); status = "Loss"; } 			// 수정 필요
	}
	
	// Response message
	public void resWrite(int scode, String value) {
		int if_write = rd.nextInt(10);
		if(if_write < 7) {
			c.resMessage(scode, value);
			status = "NoLoss";
		}
		else { c.resMessage(scode, value); status = "Loss"; } 		// 수정 필요
	}
		...
		try {
			...
			while(true) {
				...
				if(msg.equals("a")) {
					...
					sm.resWriteTest(resMessageTest(130, valueB));
				}
				else if(msg.equals("c")) {
					// 클라이언트와의 연결 시간
					String valueC = resValue(150, null);
					c.sm.resWrite(150, valueC);
				}
...
//					sm.write(300, resValue(300, null));
//					resMessage(300, resValue(300, null));
	...
	public String resMessageTest(int scode, String value) {
